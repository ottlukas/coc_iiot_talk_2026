...
> **Note**: On first run, you may need to generate the `public/` directory locally:
> ```bash
> npm install && npm run build
> ```
> This generates the bundled reveal.js assets that Docker mounts as a volume.
---
...
> **Important**: Run all commands from the **repository root** directory for correct build context.
...
```bash
docker run --rm -it -p 4200:4200 -v "$(pwd)/docs":/app/docs -v "$(pwd)/src":/app/src -v "$(pwd)/public":/app/public coc-iiot-talk-2026
```
...
```cmd
docker run --rm -it -p 4200:4200 -v "%cd%/docs":/app/docs -v "%cd%/src":/app/src -v "%cd%/public":/app/public coc-iiot-talk-2026
```

> **Note**: Volume mounts (`-v` flags) mount your local `docs/`, `src/`, and `public/` directories into the container. This allows live reloading and ensures the container can access your source files and generated output. The `public/` directory is generated by `npm run build` and must exist locally before running the container.

---

## How to Edit and Present
