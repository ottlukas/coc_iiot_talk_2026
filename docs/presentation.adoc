...
The key message is simple: compliance does not come from a database alone. It comes from the validated system around it: controlled ingestion, traceability, access control, retention, monitoring, testing, and lifecycle governance.
--

[.section-break]
== The Pressure: Connected Manufacturing Meets Regulation
...
|===
|Principle |IIoT architecture implication

|Attributable |Every reading links to a machine, sensor, gateway, service, and timestamp.
|Contemporaneous |Data is captured as close as possible to the actual machine event.
|Original |Raw values are preserved before transformation or aggregation.
|Complete |Retries, gaps, corrections, and synchronization events remain traceable.
|Enduring |Records survive retention periods, infrastructure changes, and restore scenarios.
|Available |Authorized users can retrieve data for operations, review, or inspection.
|Transparent |Processing logic, configuration, and data movement can be reviewed.
|Accountable |System and user actions are linked to responsible identities or services.
|===
...
[.notes]
--
The key point is that we do not validate open source differently from commercial software.
...
* Watch for authenticated ingestion.
* Watch for offline buffering.
* Watch for live synchronization progress.
* Watch for historical visibility in Apache IoTDB.
* Watch for idempotent resume behavior after interruption.
...
|6 |Simulate an interruption and show idempotent resume behavior.
|===
